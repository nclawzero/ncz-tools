...
**v0.3.1 — Turbo Vision UX release.** Ships the Paradox 4.5/dBASE V-inspired `turbo-vision-4-rust` TUI, real slash-command routing through `CommandHandler`, live workspace/model/context-token status, spinner/menu feedback, cached local connect splash, milestone welcome-back lines, opt-in beep-on-error, and a persisted `/resync` / `/resync --force` recovery fence for timed-out mutating slash commands.
